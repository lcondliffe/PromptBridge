...
export default function ClientShell({ children }: { children: React.ReactNode }) {
  ...
  if (pathname.startsWith("/login") || pathname.startsWith("/register")) {
    return (
      <div className="min-h-screen bg-neutral-950 text-zinc-100 flex flex-col items-center">
        ...
        <main className="w-full max-w-lg px-6 pb-8 mx-auto">{children}</main>
      </div>
    );
  }
  ...
  if (!isLoaded) {
    return (
      <div className="min-h-screen bg-neutral-950 text-zinc-100 flex items-center justify-center">
